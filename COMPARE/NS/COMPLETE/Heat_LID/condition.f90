...
do j=yin,yMax
  ...
  !Scaler BCWest
  Gun(1, xin,j)=wght(1)*TC+wght(3)*TC-Gun(3,xin,j)
  Gun(5, xin,j)=wght(5)*TC+wght(7)*TC-Gun(7,xin,j)
  Gun(8, xin,j)=wght(8)*TC+wght(6)*TC-Gun(6,xin,j)
end do
do j=yin,yMax
  !BounceBack BCEast
...
do i=xin+1,xMax-1
  ...
  Gun(4,i,yMax)=wght(4)*TH+wght(2)*TH-Gun(2,i,yMax)
  Gun(8,i,yMax)=wght(8)*TH+wght(6)*TH-Gun(6,i,yMax)
  Gun(7,i,yMax)=wght(7)*TH+wght(5)*TH-Gun(5,i,yMax)
end do
